...
namespace nscale {
    ...
    cv::Mat Normalization::segFG(cv::Mat I, cv::Mat M) {
        ...
        cv::Mat discriminant1 =
                M.at<float>(0, 0) * rd + M.at<float>(0, 1) * gd + M.at<float>(0, 2) * bd + M.at<float>(0, 3);

        //  discriminant2 = M(2,1)*double(I(:,:,1)) + M(2,2)*double(I(:,:,2)) + M(2,3)*double(I(:,:,3)) + M(2,4);
        cv::Mat discriminant2 =
                M.at<float>(1, 0) * rd + M.at<float>(1, 1) * gd + M.at<float>(1, 2) * bd + M.at<float>(1, 3);
    ...
    void
    nscale::Normalization::PixelClass(cv::Mat I, cv::Mat o_fg, cv::Mat o_bg, cv::Mat &o_fg_lab, cv::Mat &o_bg_lab) {
