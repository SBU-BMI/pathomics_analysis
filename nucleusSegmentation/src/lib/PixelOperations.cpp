...
namespace nscale {
    ...
    Mat PixelOperations::bgr2gray(const ::cv::Mat &img) {
        ...
        for (int i = 0; i < nr; i++) {
            const unsigned char *data_in = img.ptr < unsigned
            char > (i);
            unsigned char *data_out = gray.ptr < unsigned
            char > (i);
            for (int j = 0; j < nc; j++) {
    ...
    void PixelOperations::ColorDeconv(const Mat &image, const Mat &Q, const vector<float> &lut, Mat &H, Mat &E,
                                      bool BGR2RGB) {
        ...
        //int step1 = image.step1(); // step1: number of channel elements in a row.
